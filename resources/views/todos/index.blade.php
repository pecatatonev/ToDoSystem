...
<div class="max-w-4xl mx-auto p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex gap-2 items-center">
            ...
            <form method="GET" action="{{ route('todos.index') }}" class="flex gap-2 items-center ml-4">
                    ...
                    <div class="border rounded-xl p-4 flex items-start justify-between gap-4">
                        ...
                        <div class="flex flex-col gap-2 items-end">
                            ...
                            <form method="POST" action="{{ route('todos.toggle', $todo['id']) }}">
                                @csrf
                                @method('PUT')
                                <button class="text-sm px-3 py-1 rounded-lg border hover:bg-gray-50">
                            ...
                            <form method="POST" action="{{ route('todos.destroy', $todo['id']) }}" onsubmit="return confirm('Delete this todo?')">
                                @csrf
                                @method('DELETE')
                                <button class="text-sm px-3 py-1 rounded-lg border border-red-300 text-red-600 hover:bg-red-50">
                                    🗑 Delete
                                </button>
                            </form>
